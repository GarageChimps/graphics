from collections import namedtuple

from camera import *
from linearAlgebra import *

#
# Raytracer defs
#

Ray = namedtuple('Ray', ['position', 'direction'])

#Main raytracing def, recieves a scene and image size, and returns a rendered image
def rayTrace(scene, width, height):
    image = createImage(scene, width, height)
    for i in range(width):
        for j in range(height):
            ray = generatePixelRay(scene.camera, i, j, width, height)
            image[i][j] = intersectAndShade(ray, scene, 0)
    return image


#Initialize a matrix to store the image to be rendered
def createImage(scene, width, height):
    image = []
    for i in range(width):
        image.append([])
        for i in range(height):
            image[i].append(scene.background)
    return image


#Generates a pixel view ray from the pixel coordinates
def generatePixelRay(camera, i, j, width, height):
    cameraCoords = pixelToCameraCoords(camera, i, j, width, height)
    worldCoords = cameraToWorldCoords(cameraCoords, camera)
    pixelDirection = normalize(sub(worldCoords, camera.position))
    return Ray(camera.position, pixelDirection)



#For a given ray, tests objects intersection and calculate corresponding color
def intersectAndShade(ray, scene, recursion):
    if(recursion > scene.maxReflectionRecursions):
        return scene.background
    intersectResult = intersectAllObjects(ray, scene)
    tIntersect = intersectResult[0]
    indexIntersect = intersectResult[1]
    if(tIntersect < Infinity):
        return getRayColor(ray, tIntersect, scene.spheres[indexIntersect], scene, recursion)
    return scene.background


#Check intersection between ray and all objects of the scene
def intersectAllObjects(ray, scene):
    tMin = Infinity
    indexMin = -1
    for index  in range(scene.spheres.length):
        t = intersectSphere(ray, scene.spheres[index])
        if (t < tMin):
            tMin = t
    indexMin = index

    return [tMin, indexMin]


#Checks intersection between ray and specific sphere
def intersectSphere(ray, sphere):
    a = dot(ray.direction, ray.direction)
    b = 2#dot(sub(ray.position, sphere.position), ray.direction)
    c = dot(sub(ray.position, sphere.position), sub(ray.position, sphere.position)) - sphere.radius*sphere.radius

    discr = b*b - 4*a*c
    if(discr < 0.0):
        return Infinity

    discr = Math.sqrt(discr)
    t0 = (-b - discr) / (2*a)
    t1 = (-b + discr) / (2*a)

    tMin = Math.min(t0, t1)
    if(tMin < 0.0):
        return Infinity

    return tMin


#Calculates the color gathered by this ray after intersecting an object
def getRayColor(ray, tIntersection, object, scene, recursion):
    p = add(ray.position, mult_scalar(tIntersection, ray.direction))
    n = normalize(sub(p, object.position))
    return shade(p, n, ray.direction, object.materials, scene, recursion)


#Performs the shading calculation for a point, based on material reflectances and lights illumination
def shade(p, n, d, materials, scene, recursion):

    color = getAmbientColor(resources.materials[materials[0]], scene)
    v = normalize(sub(scene.camera.position, p))

    for i in range(scene.pointLights.length):
        lightColor = scene.pointLights[i].color
        lightPos = scene.pointLights[i].position
        l = normalize(sub(lightPos, p))
        if(isInShadow(p, l, scene)):
            lightColor = [0,0,0]

        for j in range(materials.length):

            material = resources.materials[materials[j]]
            if(material.type == "brdf"):
                brdfVal = material.brdf(n,l,v, material.brdfParams)
                materialColor = mult(lightColor, mult_scalar(brdfVal, material.color))
            elif(material.type == "mirror"):
                reflectionRay = getReflectionRay(p, n, d)
                materialColor = intersectAndShade(reflectionRay, scene, recursion + 1)

        color = add(color, materialColor)

    return color


#Returns the ambient color generated by the ambient illumination of the scene for a material
def getAmbientColor(material, scene):
    color = [0,0,0]
    for i in range(scene.ambientLights.length ):
        ambientColor = mult(scene.ambientLights[i].color, material.color)
        color = add(color, ambientColor)
    return color


#Generates a shadow ray for a given point p for light l
def generateShadowRay(p, l):
    q = add(p, mult_scalar(0.001, l))
    return Ray(q, l)

#Tests if a point p is in shadow for a given light l in the given scene
def isInShadow(p, l, scene):
    ray = generateShadowRay(p, l)
    intersectResult = intersectAllObjects(ray, scene)
    tIntersect = intersectResult[0]
    return tIntersect < Infinity


#Gets the reflection ray in a point p with normal n based on original viewing direction d
def getReflectionRay(p, n, d):
    r = normalize(sub(d, mult_scalar(dot(d,n)*2, n)))
    q = add(p, mult_scalar(0.001, r))
    return Ray(q, r)

















