...
namespace raytracer
{
  ...
  class Raytracer
  {
    ...
    private static Vector GetRayColor(Ray ray, float tIntersection, IObject obj, Scene scene, Resources resources, int recursion)
    {
      ...
      return Shade(p, n, ray.Direction, obj.Materials, scene, resources, tIntersection, recursion, ray.Time);
    }


    // Performs the shading calculation for a point, based on material reflectances and lights illumination
    private static Vector Shade(Vector p, Vector n, Vector d, List<string> materials, Scene scene, Resources resources, float tIntersection, int recursion, float time)
    {
      var color = new Vector ( 0, 0, 0 );
      color += GetAmbientColor(materials, scene, resources);
      color += GetShadingColor(p, n, d, materials, scene, resources, tIntersection, recursion, time);
      return color;
    }
    ...
    private static Vector GetShadingColor(Vector p, Vector n, Vector d, List<string> materials, Scene scene, Resources resources, float tIntersection, int recursion, float time)
    {
      var color = new Vector ( 0, 0, 0 );
